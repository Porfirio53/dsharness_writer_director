...
def _request_headers_for_log(headers: Any) -> dict[str, str]:
    return {
        str(key): "[REDACTED]" if str(key).lower() in _SENSITIVE_REQUEST_HEADERS else str(value)
        for key, value in headers.items()
    }


def _redact_sensitive_values(text: str, headers: Any) -> str:
    redacted = text
    for key, raw_value in headers.items():
        if str(key).lower() not in _SENSITIVE_REQUEST_HEADERS:
            continue
...
class _UsageProxyHandler(BaseHTTPRequestHandler):
    ...
    def _forward(self) -> None:
        ...
        payload, parse_error = _extract_payload(logged_response_text, response_headers)
        ...
        if isinstance(payload, dict):
            ...
            preview = logged_response_text[:200].strip()
            if preview:
